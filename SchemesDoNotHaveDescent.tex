...
\begin{prop}{\label{prop}}
	Let $2 \neq 0$. Let $r : R^\times$. Denote $C_r = R[X] / (X^2 - r)$. The following types (referred as (1.) (2.) (3.) )are logically equivalent
% https://q.uiver.app/#q=
\[\begin{tikzcd}
	{\|\Spec C\|} \\
	{\mathrm{isScheme}\left ( \underbrace{\sum_{x : R} (\Spec C_r)^{x = 0}}_{\equiv: X_r} \right)} \\
	{\sum_{Y : R \to \Sch} Y_0 =_{\Sch} \Spec C_r\times (\not \exists \text{ an open affine neigbhorhood of } Y_0 \ in \ \sum_{x : X} Y_x)}
...
\begin{proof}
	...
	First I give a more conceptual proof in the case where every scheme is modal only needing $(1.) \leftrightarrow (2.)$:
	\begin{align*}
	L1 &\rightarrow L \| \Spec C_r\| \\
	&\overset{1 \rightarrow 2}{\rightarrow} L (X_r \in \Sch) \\
	& \overset{*}{\leftrightarrow} (X_r \in \Sch ) \\
	&\overset{2 \rightarrow 1}{\rightarrow} \|\Spec C_r\| \\
	\end{align*}
where at $(*)$ we used that $(X_r \in \Sch) \simeq \sum_{X: \Sch} X =_{U_\circ} X_r$ is modal because both $X$ and $X_r$ are modal \\
	
	No we give a proof for the general case, only using $(1.) \leftrightarrow (3.)$
	If $\Sch$ is a modal type, then the type $(3.)$ is modal:
	\begin{itemize}
...
%		\item $R$ is modal (L0) , 
	...
	We have a map $\|\Spec C_r\|  \to (3.)$ from \ref{prop}. As $(3.)$ is modal we find a map $Y : L \|\Spec C_r\| \to (3.)$, By (L1) a term in (3.). Conclude by $(3.) \rightarrow (1.)$
\end{proof}
