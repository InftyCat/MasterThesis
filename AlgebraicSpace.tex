...
For this section let $G$ denote a group that is a covering 0-stack. Let $X$ be a stack equipped with a free $G$ action, i.e. for all $x , y : X$ the type 
\[
\sum_{g: G} g x = y
\]
is a proposition. 
...
\end{proof}
\begin{lemma}
	geometric $(n)$-stacks are stable by quotients of covering Group sheaves.
\end{lemma}
\begin{proof}
	The map $ X \to L_T (X / G)$ is fibered in covering 0-stacks. As $X$ is a geometric stack, the quotient is a geometric stack as well.
\end{proof}