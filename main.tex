...
\end{theorem}
\begin{proof}
	By \ref{prop:EtaleTruncations}, \ref{lemma:coveringDMstacks} we need to show that for any $x : X$, $\Omega^2(X,x)$ is formally \etale. By \ref{lemma:DMUnrIdTypes}, $x =_X x$ is formally unramified, hence its identity types are formally \etale and flat (because open). By \ref{lemma:detectCovering} $\Omega^2(X,x)$ is covering.
\end{proof}
